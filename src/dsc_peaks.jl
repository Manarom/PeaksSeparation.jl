...
begin 
	pks = Peaks.findmaxima(d)
	plot(x,d,label=nothing)
	scatter!(x[pks.indices],pks.heights,markershape=:square) 
	plot!(m_p)
end
...
begin 
	plot(all_plot_data,label = reshape(files_names,1,length(files_names)),linewidth=3,box=:on,minorgrid=:on)
	
	xlabel!(data_file_names[x_ind])
...
md" refit previous $(@bind refit_previous CheckBox())"

# ╔═╡ 8836e046-7daa-4117-ad2e-289a57b2286b
@bind refit Button("refit data")

# ╔═╡ eb08b226-e156-436e-b6ee-0ef7aaa94074
begin 
	sol_c = Ref{MultiPeaks}();
	prev_UU = Ref{NamedTuple}()
end
...
Use constraints if applicable $(Child(:use_constraints, 
	CheckBox(default=def(:use_constraints,true))))

Fitting region for $(x_coord_name) : 

$(Child(:left_x_region, Slider(x_full,show_value=true,
								default=def(:left_x_region,minimum(x_full))
	)))

$(Child(:right_x_region, Slider(x_full,show_value=true,
								default= def(:right_x_region, maximum(x_full)))))
Enable endo- peaks?
$(Child( :allow_endo,CheckBox(default=false)))
	
Enable negative basline tangent ?
$(Child( :allow_negative_basline,CheckBox(default=false)))
	"""
...
begin 
	if x_coord_name!=y_coord_name
		prev_UU[] = UU
		fl = map(j->UU.left_x_region<=j<=UU.right_x_region,x_full)
		x_data = x_full[fl]
		y_data =y_full[fl]
...
begin 
	if !isempty(show_peaks) ||  !isempty(other_shows)
		...
		if !isempty(other_shows)
			p_res = is_plotted ? p_res : plot(m.x,getfield(m,other_shows[1]), label = string(other_shows[1]),linewidth=2)
			start_from = is_plotted ? 1 : 2
			for s in other_shows[start_from:end]
				plot!(p_res,m.x,getfield(m,s), 
					 label = string(s),
					 linewidth=2)
			end
		end
...
if !isempty(trial_vect)
	md"""Values of trial runs discrepancy:
	
	$(join(string.([ sqrt(sum(t->^(t,2),p.r)) for p in trial_vect ])," --- "))
	"""
end
...
begin 
	...
	if !isempty(show_peaks) 
		...
		for i in show_peaks
			...
			counter +=1
			col = splitted_mat[:,i]
			setproperty!(data_to_replace,y_coord_name,col)
			cur_file_name = joinpath(put_file_folder,"peak_$(counter).txt")
